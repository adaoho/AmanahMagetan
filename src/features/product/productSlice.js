...
export const productSlice = createSlice({
  ...
  reducers: {
    ...
    filterByCategories: (state, action) => {
      ...
      const indexCategory = state.filteredCategories.findIndex(
        (category) => category === newItem
      );

      if (indexCategory !== -1) {
        const updatedCategory = state.filteredCategories.filter(
          (category) => category !== newItem
        );
      ...
      if (state.filteredCategories.length === 0) {
        ...
        tempDatas = state.productItems.filter((item) =>
          state.filteredCategories.includes(item.category)
        );
      }

      if (state.keywordSearch !== "") {
        tempDatas = tempDatas.filter((item) =>
          item.title.toLowerCase().includes(state.keywordSearch.toLowerCase())
        );
      }
    ...
    addWishlist: (state, action) => {
      const newItem = action.payload;
      const indexProduct = state.wishlist.findIndex(
...
export const selectFilteredCategories = (state) =>
  state.product.filteredCategories;
